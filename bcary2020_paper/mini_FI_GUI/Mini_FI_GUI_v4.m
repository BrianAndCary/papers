...
% Files matlab or igor? If Matlab files, set to 'MATLAB'. Anything else
% triggers loading file as igor binary
...
hs.sealtestend  = 550; % in ms
hs.sealteststart  = 0.1; % in s
hs.sealtestlength  = 0.4; % in s
...
hs.current_filename = {};
hs.saved_filenames = {};
hs.have_run_stored_files_yesno = 0;
hs.ax = [];
...
hs.fig = figure('Visible','on','NumberTitle','off',...
    'Color',[.8 .8 .95],'Position',[.125*hs.s_wid .350*hs.s_ht .6*hs.s_wid .48*hs.s_ht],...
...
hs.param_fig = figure('Visible','on','NumberTitle',...
    'off','Color',[.92 .8 .85],...
    'Units', 'normalized',...
    'Position',[0.07 0.25 0.15 0.45],...
    'Visible', 'off'); 
    
uicontrol(hs.param_fig,'style','text',...
    'string','Mini Finding Parameters',...
    'Units','normalized',...
...
if hs.debug_on == 1
    hs.SaveDir = '/Users/briancary/Desktop/Lab/Brian Slice Data/Temp';
else
    [hs.SaveDir] = uigetdir();
end
...
if hs.LoadPrevSave_on == 1
    ...
    try
        ...
        disp(ME)
        disp('error loading previous file saves')
    end
...
function explore_data(~, ~)
    ...
    if strcmp(hs.mat_or_igor, 'MATLAB')
        ...
        hs.data = IBWread([hs.zDir, hs.file_names(hs.trace).name]);
...
function explore_plot(data_to_plot) % This plot function accepts data in base units (V or A) no scaling
    ...
    scrollplot(data_plot)
    
    catch
        disp('explore plot error (data viewer function)')
    end
...
function mini_analysis(~,~)
    ...
    if hs.have_analyzed == 0
        [Ra, VC_Cp, VC_Rin, Vr] = get_PassProp_VC(hs.data_y_inA, hs.sealteststart, hs.sealtestlength,...
    ...
    if hs.remove_60hz_yesno
        disp('Warning! Automatically removing seal and applying 60Hz filter')
        try
        ...
        trace_toAnalyze = filtfilt(filt, hs.data_y_inA);
    ...
    if hs.mini_analysis_on == 1
        [~, ~, hs.thiscell] = mini_detector(trace_toAnalyze, hs.filters);
    end
...
function rem_seal_but(~,~)
    ...
    if hs.remove_seal_yesno == 0
        hs.data_y_inA = hs.data_y_inA((hs.sealtestend+1)...
            *(hs.samp_rate/1000):end);        
...
function plot_mini_analysis(~,~)
    ...
    disp('smoothing data shown by 5')
...
function key_catcher(~,eventdata)
    ...
    %%%%%
    if strcmp(button, '=');
        if hs.have_run_stored_files_yesno == 0;
            continue_but();
        else
            analyzed_continue_but()
        end
    end
